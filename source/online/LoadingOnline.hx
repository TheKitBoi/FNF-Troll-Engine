...
class LoadingOnline extends MusicBeatState
{
	...
	override function create()
	{
		...
		(
			function (lib)
			{
				...
				if (PlayStateOnline.SONG.needsVoices)
					checkLoadSong(getVocalPath());
				checkLibrary("shared");
				if (PlayStateOnline.storyWeek > 0)
					checkLibrary("week" + PlayStateOnline.storyWeek);
				else
	...
	{
		return Paths.inst(PlayStateOnline.SONG.song);
	}
	
	static function getVocalPath()
	{
		return Paths.voices(PlayStateOnline.SONG.song);
	}
	...
	{
		Paths.setCurrentLevel("week" + PlayStateOnline.storyWeek);
		#if NO_PRELOAD_ALL
		var loaded = isSoundLoaded(getSongPath())
			&& (!PlayStateOnline.SONG.needsVoices || isSoundLoaded(getVocalPath()))
			&& isLibraryLoaded("shared");
		
		if (!loaded)
			return new LoadingOnline(target, stopMusic);
		#end
		if (stopMusic && FlxG.sound.music != null)
			FlxG.sound.music.stop();
